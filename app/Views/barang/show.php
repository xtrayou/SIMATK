...
$angka = static function ($value): string {
    return number_format((float) $value, 0, ',', '.');
};

$badgeType = static function (string $type): string {
    return strtoupper($type) === 'IN'
        ? 'bg-success-light text-success border-success'
        : 'bg-danger-light text-danger border-danger';
};

$movementLabel = static function (string $type): string {
    return strtoupper($type) === 'IN' ? 'MASUK' : 'KELUAR';
};

$movementSign = static function (string $type): string {
    return strtoupper($type) === 'IN' ? '+' : '-';
};

$movementValueClass = static function (string $type): string {
    return strtoupper($type) === 'IN' ? 'text-success' : 'text-danger';
};
?>
...
<div class="row">
            ...
            <div class="card-body p-4">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h2 class="fw-bold mb-1"><?= esc($barang['name']) ?></h2>
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge bg-light text-dark border"><?= esc($barang['category_name']) ?></span>
                                <code class="bg-light px-2 py-1 rounded text-primary fw-bold"><?= $barang['sku'] ?></code>
                                <span class="text-muted small">• <?= $barang['unit'] ?></span>
                            </div>
                        </div>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="<?= base_url('/products/edit/' . $barang['id']) ?>" class="btn btn-warning px-4 text-white">
                    ...
                    <div class="card-body p-4 text-center">
                        ...
                        <div class="row small">
                            <div class="col-6 border-end">
                                ...
                                <p class="fw-bold mb-0"><?= $angka($barang['min_stock']) ?></p>
                            </div>
                            <div class="col-6">
                ...
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex justify-content-between mb-3 pb-2 border-bottom">
                            ...
                            <span class="fw-bold text-success">+ <?= $angka($statistik['total_masuk']) ?></span>
                        </div>
                        <div class="d-flex justify-content-between mb-3 pb-2 border-bottom">
                            <span>Total Barang Keluar:</span>
                            <span class="fw-bold text-danger">- <?= $angka($statistik['total_keluar']) ?></span>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span>Net Mutasi:</span>
                            <span class="fw-bold"><?= $angka($netMutasi) ?></span>
                        </div>
                        <div class="mt-4 d-grid gap-2">
                            <a href="<?= base_url('/stock/in?product=' . $barang['id']) ?>" class="btn btn-sm btn-outline-success">
                                <i class="bi bi-arrow-down-circle me-1"></i> Input Stok Masuk
                            </a>
                            <a href="<?= base_url('/stock/out?product=' . $barang['id']) ?>" class="btn btn-sm btn-outline-danger">
            ...
            <div class="col-12 mb-4">
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                        ...
                        <a href="<?= base_url('/stock/history?product=' . $barang['id']) ?>" class="btn btn-sm btn-light border small">
                            Lihat Semua
                        </a>
                    </div>
                                    ...
                                    <tbody class="small">
                                        <?php foreach ($riwayatStok as $mut): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge <?= $badgeType((string) $mut['type']) ?> small">
                                                        <?= $movementLabel((string) $mut['type']) ?>
                                                    </span>
                                                </td>
                                                <td class="fw-bold <?= $movementValueClass((string) $mut['type']) ?>">
                                                    <?= $movementSign((string) $mut['type']) . $angka($mut['quantity']) ?>
                                                </td>
                                                <td class="text-center fw-bold"><?= $angka($mut['current_stock'] ?? 0) ?></td>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm border-0 mb-4 h-100">
            ...
            <div class="card-body">
                <div class="mb-4">
                    <label class="text-muted small d-block mb-1">Deskripsi Barang</label>
                    <?php if (!empty($barang['description'])): ?>
                        <p class="mb-0"><?= esc($barang['description']) ?></p>
                    <?php else: ?>
                        <p class="mb-0 text-muted italic">Tidak ada deskripsi.</p>
                    <?php endif; ?>
                </div>
                <hr>
                <div class="row g-3">
